...
from tensorflow.keras.layers import Convolution2D,Flatten,Dense # for creating the layers of the model (Convolution2D- for convolutional layers, Flatten- for flattening the layers, Dense- for fully connected layers)
from tensorflow.keras.optimizers import Adam                # for optimizing the model  (Adam- Adaptive Moment Estimation - Adaptive learning rate optimization algorithm)
...
def balance_dataset(data, display=True):            # display=True to display the histogram or False to not display
    n_bins = 31                                     # number of bins  
    samples_per_bin = 2000                           # number of samples per bin (Depending on Histogram Plot)
...
def load_dataset(path, data):
    ...
    for i in range(len(data)):
        indexed_data = data.iloc[i] # get the data from the ith row
        ...
        # Append the path of the images and the steering angles   
        images_path.append(os.path.join(path,'IMG/',indexed_data[0])) # append the path of the center image to the list
        steering_angles.append(indexed_data[3]) # append the steering angle to the list
    ...
    print('\nShape of the images_path array is: ', images_path.shape)
    print('\nShape of the steering_angles array is: ', steering_angles.shape)
    print('\n')
...
def augment_dataset(image_path, steering_angle):
...
def preprocess_dataset(image):
    ...
    # Crop the image
    image = image[60:135, :, :]                                                             # crop the image to remove the sky and the car bonnet  (crop the image from 60th pixel to 135th pixel in the y-axis)
    ...
    # Apply Gaussian Blur
    image = cv2.GaussianBlur(image, (3, 3), 0)                                              # Reduce the noise in the image  

    # Resize the image
    image = cv2.resize(image, (200, 66), cv2.INTER_AREA)                                    # resize the image to 200x66 (width x height)

    # Normalize the image
    image = image/255.0                                                                      # Reduce the image pixel values from 0-255 to 0-1
...
def batch_generator(image_path, steering_angle, batch_size, Training_flag):
    ...
    for i in range(batch_size):
        ...
        if Training_flag:
            image, steering = augment_dataset(image, steering)
    ...
    return image_batch, steering_angle_batch
